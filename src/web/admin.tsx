...
export function AdminPage({ data }: { data: AdminData }) {
  return <Layout title="Admin — One Good Read" description="One Good Read 管理后台" noIndex>
    <section class="admin-shell"><header class="page-intro"><p class="edition-label">CONTROL ROOM</p><h1>系统状态</h1><p>自动运行，人工只在需要时熔断。</p></header>
      ...
      <section class="admin-panel"><h2>立即运行</h2><div class="action-row"><form method="post" action="/admin/run-daily"><button>从当前候选生成今日推荐</button></form><form method="post" action="/admin/backfill"><button class="secondary">回填首批来源</button></form><form method="post" action="/admin/run-reservoir"><button class="secondary">运行 Reservoir 批次</button></form><form method="post" action="/admin/backfill-embeddings"><button class="secondary">回填 Embedding</button></form><form method="post" action="/admin/train-preference"><button class="secondary">训练偏好模型</button></form></div></section>
      <section class="admin-panel"><h2>存储与生命周期</h2><p>已跟踪 {data.storage.objectCount} 个对象 · {(data.storage.usedBytes / 1024 / 1024).toFixed(1)} MiB / {(data.storage.limitBytes / 1024 / 1024 / 1024).toFixed(1)} GiB · 状态 {data.storage.level}</p><form method="post" action="/admin/cleanup-storage"><button class="secondary">清理过期正文</button></form></section><section class="admin-panel"><h2>Reservoir 回填</h2><p>{data.reservoir ? `最近状态 ${data.reservoir.updated_at} · ${data.reservoir.value}` : "尚无协调器运行记录。"}</p></section><section class="admin-panel"><h2>偏好模型</h2><p>{data.preferenceModel ? `${data.preferenceModel.sample_count} 条有效反馈 · 最大影响 ${(data.preferenceModel.max_influence * 100).toFixed(0)}% · ${data.preferenceModel.created_at}` : "有效反馈不足 10 条，个人模型尚未启用。"}</p></section><section class="admin-panel"><h2>来源健康</h2><div class="table-wrap"><table><thead><tr><th>来源</th><th>状态</th><th>上次扫描</th><th>连续失败</th><th>操作</th></tr></thead><tbody>{data.sources.map((source) => <tr><td>{source.name}</td><td>{source.status}</td><td>{source.last_scanned_at ?? "尚未扫描"}</td><td>{source.consecutive_failures}</td><td><form method="post" action={`/admin/backfill/${source.id}`}><button>回填</button></form></td></tr>)}</tbody></table></div></section>
      ...
      <section class="admin-panel"><h2>最近告警</h2><div class="table-wrap"><table><thead><tr><th>时间</th><th>级别</th><th>告警</th><th>投递</th></tr></thead><tbody>{data.alerts.map((alert) => <tr><td>{alert.created_at}</td><td>{alert.severity}</td><td>{alert.subject}</td><td>{alert.delivery_status}</td></tr>)}</tbody></table></div></section><section class="admin-panel"><h2>反馈</h2>{data.recommendations.map((item) => <div class="feedback-row"><div><strong>{item.recommendation_date} · {item.title}</strong><span>{item.author}{item.feedback_kind ? ` · 已反馈：${item.feedback_kind}` : ""}{item.retry_count ? ` · 重试 ${item.retry_count}/2` : ""}</span></div><form method="post" action={`/admin/recommendations/${item.id}/feedback`}><button name="kind" value="valuable">非常有价值</button><button name="kind" value="good">还不错</button><button name="kind" value="not_for_me">不适合我</button><button name="kind" value="unfinished">没读完</button><button name="kind" value="later">稍后再读</button></form></div>)}</section>
    </section>
  </Layout>;
}
