...
app.get("/health/live", async (c) => {
  ...
  try {
    ...
    const ok = db?.ok === 1;
    return c.json({ ok, checkedAt: new Date().toISOString(), reason: ok ? "database_readable" : "database_unavailable" }, ok ? 200 : 503);
  } catch { return c.json({ ok: false, checkedAt: new Date().toISOString(), reason: "database_unavailable" }, 503); }
});
...
app.get("/admin/", async (c) => {
  const [articleCounts, sources, runs, recommendations, embeddingCount, preferenceModel, storage, alerts, reservoir, simulation, simulationRows, contentReview, health, operationalCheck] = await Promise.all([
    ...
    storageUsage(c.env),
    c.env.DB.prepare("SELECT * FROM alerts ORDER BY CASE lifecycle_status WHEN 'open' THEN 0 WHEN 'acknowledged' THEN 1 ELSE 2 END,coalesce(last_seen_at,created_at) DESC LIMIT 100").all<AlertRow>(),
    ...
    businessHealth(c.env),
    readOperationalCheck(c.env),
  ]);
  const recCount = await c.env.DB.prepare("SELECT count(*) count FROM recommendations WHERE status='published'").first<{ count: number }>();
  return c.html(<AdminPage data={{ health, operationalCheck, deliveryReadiness: alertDeliveryReadiness(c.env), contentReview, automationEnabled: String(c.env.AUTOMATION_ENABLED) === "true", counts: { articles: articleCounts?.articles ?? 0, ready: articleCounts?.ready ?? 0, recommendations: recCount?.count ?? 0, failures: articleCounts?.failures ?? 0, embeddings: embeddingCount?.count ?? 0 }, preferenceModel: preferenceModel ?? undefined, storage, alerts: alerts.results, reservoir: reservoir ?? undefined, simulation: simulation ?? undefined, simulationRows: simulationRows.results, sources: sources.results, runs: runs.results, recommendations: recommendations.results }} />);
});
app.post("/admin/run-health-check", async (c) => {
  if (!c.req.header("Origin")) return c.text("Invalid origin", 403);
  if (String(c.env.AUTOMATION_ENABLED) !== "true") return c.text("Public automation is disabled", 409);
  await launchWorkflow(c.env.BACKFILL_WORKFLOW, { id: `health-manual-${crypto.randomUUID()}`, params: { healthCheck: true }, retention: { successRetention: "3 days", errorRetention: "3 days" } });
  return c.redirect("/admin/", 303);
});
app.post("/admin/alerts/:id/acknowledge", async (c) => {
  if (!c.req.header("Origin")) return c.text("Invalid origin", 403);
  const result = await acknowledgeAlert(c.env, c.req.param("id"));
  if (result === "unknown") return c.text("Unknown alert", 404);
  if (result === "inactive") return c.text("Alert is not active", 409);
  return c.redirect("/admin/", 303);
});
