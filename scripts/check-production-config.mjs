...
if (automationEnabled && vars.AI_PROVIDER !== "openai") errors.push("AI_PROVIDER must be set to the production provider before automation is enabled");
if (automationEnabled && vars.EMBEDDING_PROVIDER !== "openai") errors.push("EMBEDDING_PROVIDER must be set to the production provider before automation is enabled");
